# 경제 판단력 교과서 · MVP PRD v1.1 (편 수 정정 + 학습 영역 시안 정합화)

**대상 범위**: Stage 1 파일럿 (콘텐츠 10~25편 · 학습자 모드 + 교사 모드 동시)

- **Owner**: 창업가 본인 (단일 제작자 체제, 외주 편집·디자인 병행)
- **최종 업데이트**: 2026-05-13 (v1.1 — 편 수 정정 + 학습 영역 시안 결정 반영)
- **이전 버전**: v0.5 (2026-04-24)
- **동위 결정 이력**: `PROJECT_DECISIONS_v1.md` (Tier 1, 본 문서와 함께 단일 출처)
- **기반 리서치**: `06_ValuePropositionSheet_v2` 외 선행 문서 15종
- **영구 배제**: 과금·구독·페이월·후킹·PPL·데이터 판매 (원칙 2·3·Non-Goals)
- **시각적 테마 (Aesthetic Theme)**: 맑고 투명한 해변(Clear & Bright Beach). 단, 게이미피케이션(캐릭터, 화려한 이펙트 등) 요소 없이 기능주의적 미니멀리즘 유지.

---

## 변경 이력

- **v1.1 → v1.1.1 (2026-06-27, grill-it 착수 전 모호함 해소 — 8개 결정 확정·정합화)**
  - 결정 원장: `docs/grill/GRILL_LEDGER.md`. 하네스: `CLAUDE.md`. PRD↔Issue 정합 반영.
  - **T1 (편 수)**: 133편(권별 가변 27/25/25/31/25) 확정. 권 완주·설문 트리거는 "권당 25 고정"이 아니라 **권별 실제 편수** 기준 (FW-OX-004·CT-MOCK-001·TS-UT-008 등 정합).
  - **T2 (교사 모드)**: 범위 = 교안 PDF 다운로드 + **경량 will_reuse(+comment)**. 무거운 피드백 페이지·사전/사후 설문은 배제. `TEACHER_FEEDBACK.used_in_class` 미수집 → 실사용률 KPI(FR-KPI-006)는 DEFER, 재사용 의사 KPI(FR-KPI-007)는 유지.
  - **T3 (인증)**: **카카오 OAuth** MVP 포함(Supabase Kakao provider, 이메일/비번 병행). PII는 이메일·닉네임만(불변) — §3 예고 → **확정**.
  - **T4 (모달)**: OX 통과 시 in-page 메시지 + **학습 흔적 모달(4조건)** — §1·§4 예고 → **확정**.
  - **T5 (글자 크기)**: **14/18/22/28px 4단계** 확정(16/18/20 폐기) — Story5 AC4·NF-A11Y-003 정합.
  - **T6 (분석)**: 웹 분석 = **Vercel Analytics + Plausible**, **GA4 배제**. 제품 KPI는 내부 `event_log`(§6.4·Story2 AC4 정정).
  - **T7 (진주 색)**: 권별 tint(1권 하늘/2권 분홍/3권 노랑/4권 연두/5권 순백), 미완=권 색 점선 outline+투명 (UI 스탬프맵 #175 반영).
  - **T8 (교안 PDF)**: A4 2~3p 표준, `TEACHER_KIT.pages` 2~3 검증, 단일 PDF.

- **v0.5 → v1.1 (2026-05-13, 학습 영역 시안 정합화 — 길 A 최소 침습 변경)**
  - **편 수 정정**: 125편 → **133편** (마스터 v4 기준). 1권 27 · 2권 25 · 3권 25 · 4권 31 · 5권 25.
    - 본문 4곳, `L001~L125` 포맷 → `L001~L133` 정정.
  - **§3 인증 보강 (예고)**: 카카오 OAuth 추가 (대안 인증, 추가 PII 수집 X — 이메일·닉네임만 수집).
  - **§1 게이미피케이션 예외 조항 (예고)**: 학습 흔적 모달 허용. 조건 4가지 모두 충족 시: ① 300ms 페이드만 ② "흔적·마침" 어휘 (획득·축하 금지) ③ 사용자 자유 닫기 (ESC/backdrop/X) ④ 카카오 공유 선택적, 자동 X.
  - **§4 OX 통과 후 모달 명세 (예고)**: 서버 검증 후 in-page 메시지 + 모달 동시 표시.
  - **§6 진주 권별 색 결정**: 진주 = 흰빛 베이스 + 권별 영롱한 빛 tint. 1권 하늘 / 2권 분홍 / 3권 노랑 / 4권 연두 / 5권 순백. 미완 진주는 권 색 점선 outline + 투명.
  - 자세한 변경 근거 및 결정 이력: `PROJECT_DECISIONS_v1.md` §1.
  - **본문 REQ 본문 갱신 (인증·모달 REQ 추가, 진주 색 entity 명세 등)은 PRD v0.6 Major 승격 시 정합 예정.** 본 v1.1은 *편 수 정정 + 결정 이력 박제*의 최소 침습 변경.
...
|---|---|---|
| 역할 | 133편 순서대로 채워지는 여정의 감각 | 본인이 학습한 레슨의 궤적 기록 |
...
| AC4 | ✅ Positive | S2→S3 전환율 (랜딩→첫 영상 완시청) | **≥ 20%**. Vercel Analytics + Plausible 퍼널 리포트 (GA4 미사용). |
...
|---|---|---|---|
| **133편 커리큘럼 체계 + 고유 레슨 ID** | P1 (3.80) · 원칙 5 | ChatGPT·유튜브 대비 **완결 구조** | LESSON 스키마, 콘텐츠 제작 파이프라인 |
...
**LESSON**
- `lesson_id`: 문자열 포맷 `L001` ~ `L133`. 원칙 5 준수 — 고유·불변.
...
- **분석**: Vercel Analytics + Plausible (프라이버시 우선, GA4·추적기 미사용 — NF-SEC-003). 제품 KPI는 외부 분석이 아니라 내부 `event_log`(Supabase SQL)에서 산출 (grill-it T6)
