...
- **Data Source**: The project structure consists of a total of 5 volumes (권) and 125 episodes, strictly following the `경제판단력교과서_마스터_v3.xlsx` data.
...
- **Typography**: Use **Pretendard** universally for all elements (titles, body text, UI, labels, quiz questions) for a clean, modern, and highly readable aesthetic.
...
- **No gamification**: No points, scores, ranks, levels, badge counters, "+10pt" effects, celebration animations, or progress percentages. Only simple counts like "125편 중 3편 학습 완료" are allowed.
...
| Use | Avoid |
|---|---|
| "오늘의 영상 학습 완료" | "축하합니다! 정답입니다 🎉" |
| "125편 중 3편 학습 완료" | "진도 30% 달성!" |
| "다시 학습 후 제출해 주세요" | "아쉽네요! 다시 도전!" |
| "첫 레슨을 선택해 시작하세요" | "차근차근 함께 시작해봅시다" |
| "이전 학습 위치 2분 0초부터 다시 시작합니다" | "이어보기 START!" |

### 4. Screen-by-Screen Specifications

**Global Header & Footer**
- **Header**: Sticky top. Left: Logo "고요의 경제나루". Center: "HOME", "INDEX", "STAMP MAP". Right: Font-size control (minus/plus, 14px-28px), Color-mode toggle (Light/Dark/System), Login menu. Include a "본문으로 건너뛰기" Skip Link at the very top.
- **Footer**: Three plain lines: 1. "결제 정보를 받지 않습니다 · 광고 없음", 2. "CC BY-NC-SA 4.0 라이선스로 배포됩니다.", 3. "제작: ELLA PARK".

**Screen 1: Landing (`/`)**
- Hero (Pretendard): "경제를 이해하는 가장 차분한 방법".
- Buttons: "START" (filled accent), "LOGIN" (outline).
- 3 Info Cards (mint-blue bg) + 5 "이런 분들에게 맞춤" lines below.

**Screen 2: Term Index (`/dictionary`)**
- **Search & Sort**: A search bar at the top to quickly find terms. All terms covered in the project are listed in alphabetical order (가나다순).
- **Interaction**: Clicking a term displays a speech bubble (Tooltip/Popover).
- **Bubble Content**: Shows the episode name where the term is explained, the specific time/location in the video, and a "영상 보러가기" (Watch Video) link that navigates directly to that timestamp.

**Screen 3 & 4: Auth (`/auth/signup`, `/auth/login`)**
...
**Screen 5: Lesson Watch (`/lesson/[id]`)**
...
**Screen 6: Stamp Map (`/stamp-map`)**
- Visual metaphor: A calm walk along a beach. Lessons are "pearls" (SVG circles) connected by faint Bezier curves.
- **Scope**: MUST display ALL episodes across ALL modules simultaneously. Adjust spacing and curves to comfortably fit the complete curriculum without clutter.
- Emerald-cyan for completed, sand-colored for unstarted.
- **Critical**: Free choice. ALL cards are clickable from day one. NO locks.

**Screen 7: Teacher PDF (`/teacher/kit/[id]`)**
- A4 PDF preview pane (grayscale text, QR code, CC license) + Download button ("PDF 다운로드").

**Screen 8: Admin Dashboard (`/admin/dashboard`)**
- 7 KPI cards (Signups, L4 Completion, OX Accuracy, Media Preference, DAU, PDF Downloads, Cache Hit).
