...
## :dart: Summary
- **기능명**: [FW-AUTH-005] `updateUserPreferences()` Server Action — `accessibilityMode` (boolean) + `mediaPreference` (`VIDEO` | `TEXT` | `MIXED`) + `fontSize` (`XS` | `S` | `L` | `XL` = 14/18/22/28px) 3종 환경설정을 묶음으로 영속화
...
- [ ] `lib/contracts/user.ts` 에 Zod 스키마 `UpdateUserPreferencesRequest` 정의:
  ...
  z.object({
    ...
    font_size: z.enum(['XS', 'S', 'L', 'XL']).optional(),
...
- [ ] **마이그레이션** — User 테이블에 `fontSize` 컬럼 추가 (`accessibilityMode`, `mediaPreference` 는 CT-DB-002 에서 정의됨). default `'S'`
...
- **Given**: User(`u1`) 의 기본값 (accessibilityMode=false, mediaPreference='MIXED', fontSize='S')
- **When**: `updateUserPreferences({ accessibility_mode: true, media_preference: 'TEXT', font_size: 'XL' })` 호출
...
- **Given**: User(`u1`)
- **When**: `updateUserPreferences({ font_size: 'XL' })` 호출
...
### Scenario 7: 다기기 LWW
- **Given**: Device A 가 fontSize='XL' 로 변경 직후
- **When**: Device B 가 fontSize='XS' 로 변경
- **Then**: 최종 fontSize='XS' (최후 값 우선). updatedAt 은 B 의 시각
...
- **Given**: User(`u1`) 가 fontSize='XL' 변경 후 로그아웃
- **When**: 다음 로그인 후 페이지 진입
- **Then**: fontSize='XL' 가 자동 적용. localStorage 보조 캐시 + 서버 SSOT 동시 일치
...
- **마이그레이션**: User 테이블에 `fontSize` enum 컬럼 신규 추가. default 'S'. 기존 사용자는 기본값 자동 적용
...
- **혼합 시나리오 처리** — accessibilityMode=true 일 때 fontSize 자동 XL 강제? → **자동 강제 금지**. 사용자 자율 선택 (INV-10 정신)
- **금지**:
...
- [ ] User.fontSize 마이그레이션 추가 + 기존 사용자 default='S'
- [ ] 응답 시간 p95 ≤ 200ms 측정
- [ ] OpenAPI 명세 업데이트
