...
- [ ] **인지 장치 디자인** (게임화 방지 — REQ-FUNC-004 + '햇살이 비치는 맑고 투명한 해변가' 테마 결합):
  ...
  - 스탬프 노드 아이콘: 
    - 미완료 (학습 전): **'닫혀있는 예쁜 조개'** SVG 아이콘 사용 (부드러운 모래/파스텔 톤)
    - 완료 (학습 후): **'진주가 들어있는 열려있는 조개'** SVG 아이콘 사용 (은은하게 빛나는 진주 포인트)
...
- **When**: `/stamp-map` 접근
- **Then**: 모든 125개 노드가 '닫혀있는 예쁜 조개'로 렌더링. "125편 중 0편 학습 완료" 표기. 빈 상태 안내 노출

### Scenario 2: 정상 렌더 — 일부 스탬프 보유
- **Given**: 사용자가 L001, L003, L005 스탬프 보유
- **When**: `/stamp-map` 접근
- **Then**: 3개 노드만 '진주가 들어있는 열려있는 조개' 아이콘으로 변경. 나머지는 '닫혀있는 조개'. 카운트 "125편 중 3편 학습 완료"
