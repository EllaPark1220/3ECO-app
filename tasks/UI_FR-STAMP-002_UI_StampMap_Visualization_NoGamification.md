...
- [ ] **인지 장치 디자인** (게임화 방지 — REQ-FUNC-004 + 해변 테마 결합):
  - 맵 형태: 해안선을 따라 걷는 여정처럼 전체 에피소드가 **부드러운 곡선(Bezier curve)**으로 끊김 없이 연결됨
  ...
  - 점수·랭킹·레벨 표기 절대 금지
  - 진도율 % 도 게임화 위험 — 단순 "125편 중 3편 학습 완료" 텍스트만
...
- **When**: `/stamp-map` 접근
- **Then**: 모든 모듈·레슨 카드 grayscale 렌더. "125편 중 0편 학습 완료" 표기. 빈 상태 안내 노출

### Scenario 2: 정상 렌더 — 일부 스탬프 보유
- **Given**: 사용자가 L001, L003, L005 스탬프 보유
- **When**: `/stamp-map` 접근
- **Then**: 3개 카드만 활성 (에메랄드빛). 나머지는 미완료 모래색 톤. 카운트 "125편 중 3편 학습 완료"
